...
function CreateJoke({ user, setUser, toggleJokeFetch, setToggleJokeFetch }) {
  ...
  function handleCreateJoke(e) {
    ...
    .then(() => setToggleJokeFetch(!toggleJokeFetch));
  ...
  const renderSuccessMsg = (
    <Modal show={show} onHide={() => onClose()} centered>
      ...
      <Modal.Body>You have successfully created a joke!</Modal.Body>
      <Modal.Footer>
      ...
      <div className="container">
        ...
        <div className="row ms-5 me-5">
          <form onSubmit={(e) => handleCreateJoke(e)}>
            <label htmlFor="setup" className="fs-5">
              Setup:{" "}
            </label>
            <br />
            <input
            ...
            />
            <br />
            <label htmlFor="punchline" className="fs-5">
              PunchLine:{" "}
            </label>
            <br />
            <input
              ...
              className="form-control border border-2 border-dark"
              value={newPunchLine}
              placeholder="Type your answer to joke..."
              onChange={(e) => setNewPunchLine(e.target.value)}
            />
            <br />
            <button
              type="submit"
              className="btn btn-warning border border-2 border-dark fs-5"
            >
              Create Joke
            </button>
          </form>
